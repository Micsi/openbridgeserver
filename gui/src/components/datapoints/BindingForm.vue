<template>
  <form @submit.prevent="submit" class="flex flex-col h-full">
    ...
    <div class="flex border-b border-slate-700 mb-4 shrink-0">
      ...
      >
        {{ tab.label }}
        <span v-if="tab.badge" class="ml-1.5 w-1.5 h-1.5 rounded-full bg-blue-400 inline-block" />
      </button>
    </div>
    ...
    <div class="flex-1 overflow-y-auto min-h-0 flex flex-col gap-4 pr-1">
      ...
      <template v-if="activeTab === 'transform'">
        ...
        <!-- Vorschau -->
        <div v-if="formulaPreview !== null" class="p-3 bg-slate-800/60 rounded-lg text-sm font-mono flex items-center gap-3">
          <span class="text-slate-400">x = 100 →</span>
          <span class="text-green-400 font-semibold">{{ formulaPreview }}</span>
        </div>
        <div v-else-if="form.value_formula?.trim()" class="p-3 bg-red-500/10 border border-red-500/30 rounded-lg text-sm text-red-400">
          Ungültige Formel — wird beim Speichern abgelehnt.
        </div>
      </template>
    ...
    <!-- Fixe Fussleiste -->
    <div class="shrink-0 border-t border-slate-700 pt-3 mt-3 flex items-center justify-between gap-3">
      <div class="flex items-center gap-2">
      ...
      <div class="flex items-center gap-3">
        <div v-if="error" class="text-sm text-red-400 max-w-xs truncate" :title="error">{{ error }}</div>
...
const hasTransform = computed(() => !!form.value_formula?.trim())
const hasFilter    = computed(() =>
  form.throttle_value > 0 || form.send_on_change ||
  form.send_min_delta > 0 || form.send_min_delta_pct > 0
)
const canFilter = computed(() => form.direction === 'DEST' || form.direction === 'BOTH')

const visibleTabs = computed(() => {
  const tabs = [{ id: 'conn', label: 'Verbindung', badge: false }]
  if (selectedAdapterType.value) {
    tabs.push({ id: 'transform', label: 'Transformation', badge: hasTransform.value })
  }
  if (selectedAdapterType.value && canFilter.value) {
    tabs.push({ id: 'filter', label: 'Filter', badge: hasFilter.value })
  }
  return tabs
})

// Wenn aktiver Tab verschwindet → zurück zu Verbindung
watch(visibleTabs, tabs => {
  if (!tabs.find(t => t.id === activeTab.value)) activeTab.value = 'conn'
})

// ---------------------------------------------------------------------------
// Formel-Vorschau (client-side, vereinfacht)
// ---------------------------------------------------------------------------

const formulaPreview = computed(() => {
  const f = form.value_formula?.trim()
  if (!f) return null
  try {
    // Einfacher Sanity-Check mit x=100
    const x = 100  // eslint-disable-line no-unused-vars
    // Nur einfache arithmetische Ausdrücke; für Sicherheit kein eval im Prod-Build gedacht
    // eslint-disable-next-line no-new-func
    const result = new Function('x', `"use strict"; return (${f})`)(100)
    if (typeof result !== 'number' || !isFinite(result)) return null
    return Number.isInteger(result) ? result : result.toFixed(4).replace(/\.?0+$/, '')
  } catch {
    return null
  }
})
...
function buildConfig() {
  ...
  if (type === 'MODBUS_TCP' || type === 'MODBUS_RTU') {
    return {
      unit_id: cfg.unit_id, register_type: cfg.register_type,
      address: cfg.address, count: cfg.count, data_format: cfg.data_format,
      scale_factor: cfg.scale_factor, byte_order: cfg.byte_order,
      word_order: cfg.word_order, poll_interval: cfg.poll_interval,
    }
  }
  if (type === 'MQTT') {
  ...
  try {
    ...
    if (props.initial) {
      ...
      if (!form.adapter_instance_id) {
        error.value = 'Bitte eine Adapter-Instanz wählen'
        saving.value = false
        return
      }
