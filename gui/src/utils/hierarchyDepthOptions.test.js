...
describe('buildDepthOptions — Edit-Modus mit Daten', () => {
  it('benutzt tree.name als Label für Ebene 0 (Hierarchiename) und distinct-count für tiefere Ebenen', () => {
    ...
    expect(opts).toEqual([
      { value: 0, label: '0 — Salvusbrunnen (Hierarchiename)',                              disabled: false },
  ...
  it('disabled Ebene 1+, wenn die Hierarchie keine Wurzelknoten hat', () => {
    const tree = { id: 4, name: 'Leere Hierarchie' }
    const opts = buildDepthOptions({ isEdit: true, tree, rootNodes: [], maxDepth: MAX_DEPTH })

    expect(opts[0]).toEqual({ value: 0, label: '0 — Leere Hierarchie (Hierarchiename)', disabled: false })
  ...
  it('akzeptiert undefined/null rootNodes (Hierarchie noch nicht geladen) ohne Crash', () => {
    ...
    expect(opts[0].label).toBe('0 — Noch nicht geladen (Hierarchiename)')
...
describe('buildDepthOptions — Create-Modus', () => {
  it('liefert generische Labels mit dem heutigen Wortlaut und alle Ebenen aktiv', () => {
    ...
    expect(opts).toEqual([
      { value: 0, label: '0 — Hierarchiename (Standard, z.B. "Gebäude")', disabled: false },
