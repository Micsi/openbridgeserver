 ...
 *
 * `optionen.tree` reicht eine BEREITS GELADENE Knotenliste herein. Der
 * Seitenbaum des Editors (Teil C1) liest sie ohnehin; ohne diesen Weg holte der
 * Entwurf denselben Baum ein zweites Mal, und die beiden Listen koennten in der
 * Reihenfolge der globalen Inkludeseiten (R10) auseinanderlaufen. Fehlt die
 * Option, liest der Entwurf den Baum wie bisher selbst.
 */
export async function loadDraftNodes(pageId, optionen = {}) {
  const tree =
    typeof optionen.tree === 'function' ? await optionen.tree() : (await visuApi.tree()).data
  const configs = {}
 ...
 * @param {{tree?: () => Promise<Array>}} optionen Siehe `loadDraftNodes`.
 */
export function useVisuEditorDraft(pageId, optionen = {}) {
  ...
  async function reload() {
    ...
    error.value = null
    try {
      nodes.value = await loadDraftNodes(pageId.value, optionen)
