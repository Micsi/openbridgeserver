...
| `obs/api/v1/logic.py` | `comment` excluded from layout-only diffs (as is the cosmetic `data.label` block name of every node); `datapoint_read`/`datapoint_write` for usage reporting; `missing_node` placeholder on import | API-level concerns (change detection, usage lists, import fallback) |
