...
def make_executor(
    ...
    return GraphExecutor(flow=flow, hysteresis_state=hysteresis_state, app_config=app_config or {})
...
def edge(source: str, target: str, source_handle: str = "out", target_handle: str = "in") -> dict:
    return {
        "id": f"{source}-{target}",
        "source": source,
        "target": target,
