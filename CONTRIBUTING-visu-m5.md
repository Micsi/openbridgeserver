...
| C1 Editor: Baum + Seiteneigenschaften (Micsi/openbridgeserver#168) | 2 + Nachzug | Kritiker: **Messlatte erreicht**. E9/E15/E19 mit Wirkung belegt (Speicher-Mutationen sterben), zwölf Backend-Ablehnungen vorweggenommen inkl. Kurzschluss-Reihenfolge. R1 zurückgewiesen (E19 lebte nur im Browser, E9/E15 klickten nie „Speichern", „Inkludeseite" fiel still auf normal zurück). Gefunden: `getByText('Gespeichert')` traf als Teilstring die Einleitung „Gespeichert wird dabei nichts" und war erfüllt, **bevor** gespeichert wurde; nach dem Entschlacken war die Erfolgsmeldung überhaupt keine Schranke mehr. E19 ehrlich blockiert bis zum C2-Feld, Naht vom Kritiker nachgestellt und grün ohne eine Zeile gui/ | **durch** |
| C2 Editor: WYSIWYG-Canvas (Micsi/openbridgeserver#169) | 3 | Kritiker: **Messlatte erreicht**, am laufenden Stapel gemessen: eine responsive Seite liefert im Skin leere Stil-Attribute, während die Koordinaten in der Spalte stehen bleiben; kein Renderweg umgeht die Entscheidung. R1 zurückgewiesen (Seiteneigenschaften auf jedes Widget gespiegelt = Datenfork; **E17 blieb grün, obwohl das Speichern abgeschaltet war**), R2 zurückgewiesen (**R17 in der Wirkung gebrochen**: responsive Seite trug leere Koordinaten, V1 rechnete auf Breite null). R3: `layout_mode` entscheidet im Host, Koordinaten bleiben; V1-Formel aus der unveränderten Datei gelesen und nachgerechnet, V1-Suite 332 Tests grün. Liefert `PageConfig` mit `layout_mode`/`grid`/`breakpoints`/**`skin`** (letzteres schaltet C1s E19 scharf) | **durch** |
| C3 Editor: Widget-Palette + Bindung (Micsi/openbridgeserver#170) | 4 + 2 (Nachbesserung) | Kritiker: **Messlatte erreicht**. Formulare aus Vertrag, `mapping.ts` (als Modul geladen, mitschreibende Falle) und V1-Verzeichnis abgeleitet, fällt bei Quelldrift. E16 wirkt jetzt im **Host** statt im Editor (sonst zeigte die Vorschau anderes als die Visu) und live in beiden Nutzerlagen: angemeldet ~0,1 s, Gast im 4-s-Takt, auch auf Include- und Popup-Ebenen. R3 deckte auf, dass ein geerbter Verbindungsfehler E16 für Angemeldete taub machte; die geschärfte Attrappe (CONNECTING/OPEN/CLOSED, wirft im Aufbau) machte **9 Proben rot, 6 davon vorbestehende**, die zuvor über der kaputten Naht grün standen **Nachbesserung nach dem Abschluss:** Teil D fand, dass der Autorenteil **gar keinen Schreibweg** hatte - Name, Bindung, Sichtbarkeitsregel und jedes Palette-Widget gingen verloren, und „Speichern\" quittierte trotzdem Erfolg. Betroffen war jedes Feld des Bindungsformulars. R1 der Nachbesserung schloss das (kein vierter Schreiber, Quittung vergleicht jedes Element ganz), erzeugte aber denselben Fehler an neuer Stelle: Änderungen aus der JSON-Ansicht wurden still verworfen. R2 löst es mit einer **Schranke vor dem Schreiben** - weicht die Nutzlast vom Stand der Textansicht ab, wird gar nicht geschrieben. Damit 37 laufende Szenarien | **durch** |
...
| C5 Editor: Ergonomie (Micsi/openbridgeserver#172) | 3 | Kritiker: **Messlatte erreicht**. Mehrfachauswahl per Rahmen (Maus und Finger), Maus-Gruppenzug, Copy/Paste/Duplizieren seitenübergreifend, Undo/Redo als Zustandsstapel (Deckel 50, exakte Wiederherstellung inkl. Auswahl), Pfeiltasten-Nudging, Touch. R1 und R2 zurückgewiesen: beide Blocker waren im Browser sichtbar, aber von **keinem Szenario berührt** - ein Zug an einer ausgewählten Kachel bewegte nur diese und verwarf die Auswahl still; der Anfasser verdeckte eine 3x2-Kachel ganz und kaperte die Nachbarn. R2s Reparatur schoss über das Ziel hinaus (Rückzug auch vor Kacheln **hinter** der eigenen: im lückenlosen Raster **0 von 64** Griffpixeln erreichbar). R3: Rückzug nur bei Z-Ordnung **und** vollständiger Verdeckung, dazu `z-10` (ohne diese Zeile war der Griff im Raster gar nicht anklickbar); Raster-Szenario ergänzt, damit der Normalfall künftig geprüft wird | **durch** |
| C6 Editor: JSON-Dualität + Versionen (Micsi/openbridgeserver#173) | 3 | Kritiker: **Messlatte erreicht**. Eigene Versionstabelle (Migration 54), ausdrücklich **kein** Copy-Rename (dessen DROP löscht per Cascade authz-Policies und PIN-Hashes, Lehre aus Teil A). Kein dritter Schreiber: Wiederherstellen liest den alten Stand und geht durch das normale Speichern; die Quittung erscheint erst nach dem Nachziehen. R1 zurückgewiesen (ein **fünfter** Schreibweg schrieb keine Version: der oberste Eintrag war weder ausgeliefert noch wiederherstellbar), R2 zurückgewiesen (ein **sechster**, `POST /config/import` als UPSERT, machte den Import beim Wiederherstellen **still** rückgängig, erreichbar über die Sicherungswiederherstellung). R3 löst es byteweise verglichen und erst nach dem Speichern. Dazu: Export mit Emoji brach mit 500, E18 prüfte nur Namen, zwei stille Verluste beim Import sind jetzt laut, PIN-Verlust fällt fehlerschließend zu | **durch** |
| D Admin + Doku + i18n (Micsi/openbridgeserver#174) | 2 | Kritiker: **Messlatte erreicht**. Ausliefer-Route der V2-Visu (`/visu-v2`), Editor-Bereich hinter dem Admin-Login, 4 Hilfeseiten je Sprache mit 17 Ankern (Doku-Gate #1183), i18n paritätisch. Schaltete E11 und E16 frei → **36 Szenarien**. R1 zurückgewiesen mit drei Blockern, die alle beim ersten Ausprobieren aufgefallen wären: das Ergebnis kam in **keinem** ausgelieferten Paket an (Docker, LXC, Update-Bündel), ein **frisch angemeldeter Admin** kam gar nicht in den Editor (Anmeldezustand aus dem Speicher abgeleitet statt reaktiv; der gelieferte Wächter konnte es nicht sehen), und die deutschen Hilfeseiten waren ohne Umlaute geschrieben (208 Ersatzschreibungen). R2 löst alle drei; der Kritiker hat das Abbild **mit und ohne** vorgebautes Bündel selbst gebaut und R17 über beide per Prüfsumme belegt. Nebenbei: ein Skript-Fehler machte LXC- und Bündel-Bau auf macOS unfahrbar | **durch** |
...
| Integrationsrunde (E gegen A+B+C) | 1 | **38 von 39 Szenarien grün**, nur E3 (Pixel-Diff) bleibt ausgesetzt. Auf dem zusammengeführten Stand selbst gefahren, eigener Portstapel | **durch** |

### Folge-Wellen (die Issues aus den Kritiken, drei parallele Gruppen)

Alle drei mit getrenntem Kritiker; jede vom Kritiker **überlebende Mutation** wurde vor dem Zusammenführen mit einer Probe geschlossen, statt sie als geringfügig abzulegen.

| Gruppe | Issues | Urteil des Kritikers | Nachgezogen | Stand |
|---|---|---|---|---|
| F1 Release-Paket | Micsi/openbridgeserver#191 | **nicht erreicht**: der Kommentarkopf von `tools/build-visu-v2.sh` behauptete weiter, den CI-Werkstücken fehle der Checkout von `obs-visu-skins` - ausgerechnet in dem Skript, das die CI aufruft, und beim Nachziehen der sieben anderen Doku-Stellen übersehen. Sonst kein Pfad gefunden, auf dem ein Paket ohne `visu_v2_dist/` entsteht (kein `always()`, kein `continue-on-error`, keine Cache-Umgehung; `nightly-lxc.yml` erbt den Fix über den Dispatch) | Kopf berichtigt und die Fehlerfall-Entscheidung aufgenommen (`1b467fae`); Checkout **und** Aufruf in allen drei Werkstücken je zweifach belegt | **durch** |
| F2 Editor-Rückfall + Wettlauf | Micsi/openbridgeserver#187, #188 | **nicht erreicht**: das live gemessene Verhalten war in allen drei Abläufen richtig (kein vierter Schreiber, genau ein Schreibvorgang je Klick), aber **3 von 9 Mutationen überlebten**. Die drei Feld-Schranken in `confirmed()` liessen sich streichen, ohne dass ein Test rot wurde (`lcov`: 0 Treffer auf dem Erkennungszweig); der #188-Wettlauftest hing am **Wortlaut** der Bestandsabfrage und feuerte gegen die alte Fassung gar nicht erst; der Reihenfolge-Test behauptete mehr, als er zeigt | Je eine Probe pro Feld (einzeln mutiert tötet jede genau ihre eigene, die übrigen sieben bleiben grün); Wettlauftest vom Wortlaut gelöst und mit Vorbedingung versehen, gegen den zurückgedrehten Fix rot; Reihenfolge-Test nach der Zusage benannt, die seit der Transaktions-Klammer wirklich trägt (`cd20351f`) | **durch** |
| F3 Host-Restpunkte | Micsi/openbridgeserver#184, #180, #186 | **erreicht**. Keine zweite Bahn zum Popup: sechs Randfälle geprüft (unbekannte Id, Array-Query, Remount, PIN-Seite - dort stumme Leerseite statt Gate, kein Datenleck, weil das Backend `page_config` ohnehin verdeckt). R7/R8 selbst nachgemessen, #186-Mutation rot bei entferntem **und** bei doppeltem Nachsenden. **E2E vom Kritiker selbst gefahren: 38/1/0.** 2 von 6 Mutationen überlebten | Einmal-Flag des Deep-Links (sonst wird `?popup=` vom Befehl zum Dauerauftrag: `refresh()` nach Login reisst das geschlossene Popup wieder auf) und das Vergessen einer geschlossenen Id (sonst reisst ein Unmount ein inzwischen **fremdes** Popup mit) - beide gegen ihre Mutation rot gefahren (`a6d5ebd0`) | **durch** |

### Zweite Folge-Welle (G1-G3)

| Gruppe | Issues | Runden | Ergebnis |
|---|---|---|---|
| G1 Backend-Validierung | #176, #177, #178 | 4 | #178 gefixt, #177 als Testlücke geschlossen (kein Verhaltensfehler). **#176 nach drei Reparaturrunden ersatzlos zurückgebaut** - jede Runde schloss den gemessenen Befund und brachte einen neuen an derselben Stelle: Runde 1 vier weitere Ausgänge offen **+ neuer Datenverlust**, Runde 2 Autor mit 400 blockiert, Runde 3 echter Datenverlust beim zweiten Speichern. Ursache war der Ansatz selbst (beim Schreiben muss geraten werden, was der Client beim Lesen sah). Der Platzhalter-Weg wurde entworfen, am echten Editor-Pfad geprüft und begründet verworfen (Vertragsänderung). **Issue bleibt offen, Entscheidung beim Eigentümer** (§7) |
| G2 Vorschau | #182, #183 | 5 | **Acht durchgelassene Zaun-Formen gefunden.** Runde 1 meldete Rot-Läufe gegen eine **präparierte statt echte** Vorgängerfassung - nachgemessen war #183 seit #169 bereits behoben. Formen 1-6 einzeln geschlossen, bis die in Runde 4 neu gebaute Funktion selbst Form 7 und 8 mitbrachte. Erst da war die Klasse benannt: auf 916 Zeilen ein handgebauter HTML/CSS-Parser. Runde 5 setzte an der kritischsten Stelle den echten `DOMParser` ein und wies am echten Vue-SFC-Compiler nach, dass zwei frühere Befunde auf einer **widerlegten Annahme** beruhten - der Kritiker nahm seinen eigenen Befund zurück |
| G3 GUI-Laufzeit | #185, #189 | 1 | Messlatte erreicht. WS-Abos werden nach Wiederaufbau erneuert; die Probe, die bisher das **stille Verwerfen zementierte**, hält jetzt das Richtige fest. Z-Ordnungs-Regel und JSON-Undo-Schritt gepinnt; #189.2 begründet nicht geändert |

**Gates auf dem Gesamtstand** (`1c86b17c`, alle drei Gruppen, selbst gefahren): build/typecheck/lint grün, boundaries 0 Verstösse (211 Module, 743 Abhängigkeiten), `@obs/visu-app` 49 Dateien / 774 Tests, `gui` 213 Dateien / 3258 Tests, Python **8070 passed / 0 failed**, E2E **38 passed / 1 skipped (E3) / 0 failed**.

**Gates auf dem zusammengeführten Stand** (`31adc99a`, konfliktfrei): build/typecheck/lint grün, boundaries 0 Verstösse (211 Module, 743 Abhängigkeiten), `@obs/visu-app` 49 Dateien / 774 Tests, `gui` 211 Dateien / 3211 Tests, Python **8065 passed / 0 failed**, E2E **38 passed / 1 skipped (E3) / 0 failed**.

> Zur Umgebung: die Container-Tests scheitern hier reihenweise, solange `TMPDIR` nicht auf `/Volumes/Daten/tmp` zeigt - Docker ist auf dieser Maschine Colima, und nur dieser Pfad ist in die VM gemountet (steht in `apps/visu/e2e/README.md`, Schritt 1). Mit richtigem `TMPDIR` sind es 0 Fehler statt 22.
