...
### Fixes 🐞
* Backend/KNX: `BOTH`-direction bindings now recognize and individually consume their own short-lived command and state-address confirmations by binding and raw payload, including confirmations from rapid consecutive writes and XKNX automatic reconnect retries. Coalesced state-address feedback for identical encoded payloads restores the newest logical command, while command-address callbacks remain ordered. The confirmation window begins only after XKNX transmits a queued telegram; failed or expired transmission state and reloaded bindings are cleaned automatically. Confirmations restore the pre-transformation logical value to persisted/UI state but bypass further binding propagation, preventing non-idempotent `value_formula` transformations from creating runaway feedback loops while leaving unrelated bindings and later genuine value changes unaffected. https://github.com/abeggled/openbridgeserver/issues/910
