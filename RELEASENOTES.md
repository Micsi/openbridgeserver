...
* Logic Engine/Admin GUI: Added a Change Filter node (Edomi-style "SendByChange") that outputs the input value and fires a changed trigger only when it differs from the last received value, so repeated identical values can be suppressed anywhere in a graph — e.g. wired into a Write Object node's trigger input to avoid redundant writes. https://github.com/abeggled/openbridgeserver/issues/1087
