...
### New features ✨
* Backend/AuthZ: Completed scoped creation authority for Visu, Logic, and adapter-owned operations. Visu create/copy/import now derives authority from an existing target parent; Logic create/import/duplicate uses a closed user-only capability and persists delegated graphs disabled with a bounded creator grant; adapter operations require explicit code-declared capabilities. Root resources and adapter instances remain admin-only, and API keys gain no new creation authority. https://github.com/abeggled/openbridgeserver/issues/583
* Backend/Admin GUI: Added `room_local` and `central_plant` authorization classes with an explicit per-scope central-control switch, enforced them for DataPoint writes and Logic runs, and extended central policy checks to existing Logic/Visu mutations and RingBuffer filterset references. https://github.com/abeggled/openbridgeserver/issues/583
* Backend/Admin GUI: Consolidated Visu page, RingBuffer filterset, and adapter-instance access into the central authorization model with one-time, fail-closed migrations; legacy page PIN hashes are no longer stored in grants, audits, or exports. https://github.com/abeggled/openbridgeserver/issues/629
* Backend/Admin GUI (Upstream PR #991): Added secure explicit first-owner bootstrap and offline recovery, atomic user-artifact transfer and API-key revocation during account deletion, and narrowly scoped admin-approved API-key capabilities for Visu page and datapoint metadata configuration.
* Backend/Admin GUI (Upstream PR #988): Added fail-closed Logic graph and side-effect capabilities, adapter delegation declarations, audited user/settings mutations, and server-backed action preflight before delegated graph runs.
* Backend/Admin GUI (Upstream PR #984): Added owner-first role-scope editing with atomic audited grant persistence, effective permission previews, and conflict-safe ETag updates.
